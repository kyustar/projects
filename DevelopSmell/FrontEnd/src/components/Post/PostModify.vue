<template>
   <div class="mt-16">
                  ...
                  ></v-text-field>
                    ...
                    <v-row>
                        <v-chip class="ma-1"
                                ...
                                :key="index" @click="removetag(index)">
                                {{tag}}
                        </v-chip>
                    </v-row>
                <v-tooltip bottom>
       ...
       <v-file-input
                ...
                <editor  id="editor" ref="toastuiEditor" height="900px" v-if="editorText != null"  :initialValue="editorText"> </editor>
                <div style="text-align:right">
                    <v-btn 
                        class="ma-2"
                        color="rgb(240, 162, 46)"
                        @click="createAction(0)"
                        >
                        임시저장
                    </v-btn>
                    <v-btn 
...
export default {
    ...
    created(){
         let postno =  this.$route.params.postno;
            ...
            axios({
                method : 'GET',
                url :this.$baseurl+'/post/detail/'+postno+"/"+this.$store.state.userm.userno
            }).then((res)=>{
    ...
    methods: {
      ...
      createAction(psave) {
        if(this.checkInput()){
            ...
            formData.append('content',  this.$refs.toastuiEditor.invoke("getMarkdown"));
            axios({
                method : 'PUT',
